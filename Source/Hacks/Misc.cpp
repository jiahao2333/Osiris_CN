...
void Misc::drawBombTimer(const Memory& memory) noexcept
{
    ...
    std::ostringstream ss; ss << "炸弹在 " << (!plantedC4.bombsite ? 'A' : 'B') << " : " << std::fixed << std::showpoint << std::setprecision(3) << (std::max)(plantedC4.blowTime - memory.globalVars->currenttime, 0.0f) << " s";
    ...
    if (plantedC4.defuserHandle != -1) {
        ...
        if (plantedC4.defuserHandle == GameData::local().handle) {
            ...
            std::ostringstream ss; ss << defusingPlayer->name << " 正在拆弹: " << std::fixed << std::showpoint << std::setprecision(3) << (std::max)(plantedC4.defuseCountDown - memory.globalVars->currenttime, 0.0f) << " s";
...
void Misc::voteRevealer(const ClientInterfaces& clientInterfaces, const Interfaces& interfaces, const Memory& memory, const GameEvent& event) noexcept
{
    ...
    memory.clientMode->getHudChat()->printf(0, " \x0C\u2022Osiris\u2022 %c%s\x01 已投票 %c%s\x01", isLocal ? '\x01' : color, isLocal ? "你" : entity.getPlayerName(interfaces, memory).c_str(), color, votedYes ? "是" : "否");
}
...
void Misc::onVoteStart(const ClientInterfaces& clientInterfaces, const Interfaces& interfaces, const Memory& memory, const void* data, int size) noexcept
{
    ...
    memory.clientMode->getHudChat()->printf(0, " \x0C\u2022Osiris\u2022 %c%s\x01 发起投票 (\x06%s\x01)", isLocal ? '\x01' : '\x06', isLocal ? "You" : entity.getPlayerName(interfaces, memory).c_str(), voteName(voteType));
}
