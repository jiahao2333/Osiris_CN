...
void GUI::renderAimbotWindow(Config& config, bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("自动开枪", &config.aimbot[currentWeapon].autoScope);
    ...
    ImGui::SliderFloat("最大开火误差", &config.aimbot[currentWeapon].maxShotInaccuracy, 0.0f, 1.0f, "%.5f", ImGuiSliderFlags_Logarithmic);
    ...
    ImGui::Checkbox("连续开火", &config.aimbot[currentWeapon].betweenShots);
    ImGui::Columns(1);
    if (!contentOnly)
        ImGui::End();
}
...
void GUI::renderTriggerbotWindow(Config& config, bool contentOnly) noexcept
{
    ...
    ImGui::SliderInt("射击延迟", &config.triggerbot[currentWeapon].shotDelay, 0, 250, "%d ms");
    ...
    ImGui::SliderFloat("持续时间", &config.triggerbot[currentWeapon].burstTime, 0.0f, 0.5f, "%.3f s");
