...
void InventoryChanger::drawGUI(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        if (!ImGui::Begin("库存修改", &windowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
            ImGui::End();
            return;
        }
    }
    ...
    if (!isInAddMode && ImGui::Button("添加项目.."))
        isInAddMode = true;
    ...
    if (isInAddMode) {
        ...
        const bool filterChanged = ImGui::InputTextWithHint("##search", "搜索武器皮肤、印花、匕首、手套、音乐盒..", &filter);
        ImGui::SameLine();
        const bool addingAll = ImGui::Button("添加所有到仓库");
